...
def process_video(yolo_path, video_path, target_fps, last_frame, batch_size=16):
    ...
    if last_frame == -1 or last_frame > total_frames:
    ...
    for i, frame in enumerate(video):
        ...
        if len(batch_frames) == batch_size or i == last_frame:
            for idx, frame in batch_frames:
                ...
                player_tracks = Converter.TrackedObjects_to_Detections_nor(player_track_objects, cls=1)
                ball_tracks = Converter.TrackedObjects_to_Detections_nor(ball_track_objects, cls=0)
                if not ball_tracks:
