...
def process_field_transformation(precomputed_results, calibrator_cfgs):
    ...
    for frame_idx, (frame, _, detections) in enumerate(precomputed_results):
        ...
        print(f"Processing frame {frame_idx} with {len(detections)} detections")
        ...
        for detection in detections:
            ...
            if class_id in [1, 2]:  # players (already classified)
                ...
                if field_pos:
                    ...
                    print(f"Added player with class_id {class_id} at field position {field_pos}")
            elif class_id == 0:  # ball detection
                field_pos = transformer.image_to_field_point(bbox)
                if field_pos:
                    ball_field_position = field_pos
                    print(f"Added ball at field position {field_pos}")
        ...
        print(f"Frame {frame_idx} result: {len(players_with_field)} players, ball: {ball_field_position is not None}")
