...
def extract_player_colors(image, bounding_boxes):
    ...
    for box in bounding_boxes:
        ...
        # Process only player boxes (assuming classes in range 1-5 are players)
        if cls in range(1, 6):
...
# -------------------------
def main_multi_frame(debug=False):
    results = []
    ...
    # If debug is True, create folders for team images
    if debug:
    ...
    for frame_idx, (frame, ball_detections, updated_boxes) in enumerate(updated_results):
        # Save cropped player images into team folders if debug is True.
        if debug:
            for det_idx, box in enumerate(updated_boxes):
                team_label = box[0]  # team label assigned from clustering (1 or 2)
                # Assuming box format: [team_label, x1, y1, x2, y2]
                if team_label in [1, 2]:
                    _, x1, x2, y1, y2 = box
                    x1, y1, x2, y2 = map(int, [x1, y1, x2, y2])
                    crop = frame[y1:y2, x1:x2]
                    folder = "team_1" if team_label == 1 else "team_2"
                    file_path = os.path.join(folder, f"frame{frame_idx}_det{det_idx}.jpg")
                    cv2.imwrite(file_path, crop)
        
        player_colors, other_boxes = extract_player_colors(frame, updated_boxes)
        team_labels = []
        for box in updated_boxes:
            # For visualization, mapping: if box[0] (team label) is > 2 leave it; otherwise adjust for display
            if box[0] in [1, 2]:
...
if __name__ == '__main__':
    # Set debug=True to enable saving of team folders and output player crops.
