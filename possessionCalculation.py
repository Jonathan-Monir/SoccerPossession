def CalculatePossession(
    ...
    for i, (frame, players, ball) in enumerate(data):
        # normalize “no detections” to None
        if not players:
            players = None
        if isinstance(ball, list) and len(ball) == 0:
            ball = None

        # if we have no players at all, or first frame no ball ⇒ skip
        ...
        # choose single ball if list of detections
        if isinstance(ball, list):
            # if we have a valid prevBall in bounds, pick the closest
            if (
                prevBall
                ...
                curr = ball[0]
                best_d = DistanceBetweenObjects(curr["field_position"], prev_pos)
                for det in ball[1:]:
        ...
        # if ball missing now, try reuse prevBall
