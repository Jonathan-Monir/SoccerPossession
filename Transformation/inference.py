...
# 
    ...
    input_path = r"examples\tactical-cam-angle_3340085.jpg"
    input_type = "image"
    model_kp = "SV_kp"
    model_line = "SV_lines"
    save_path = r"examples\ssi_results2.png"
#     device = args.device
    display = "store_true"
    kp_threshold = 0.1486
    ...
    loaded_state = torch.load("SV_kp", map_location=torch.device('cpu'))
    model = get_cls_net(cfg)
    model.load_state_dict(loaded_state)
    model.eval()

    loaded_state_l = torch.load("SV_lines", map_location=torch.device('cpu'))
...
#     process_input("examples/messi_sample.png", "image" , "SV_Kp", "SV_lines", 0.1486, 0.3880, "examples\messi_results2.png", "store_true")
